...
export class FontChange {
  ...
  attach(menuInstance) {
    const menu = document.getElementById(menuInstance.id);
    if (!menu) return;

    FontSize.createItems(menu);
    this.setCustomListeners(menu, menuInstance);
  }

  setCustomListeners(menu, menuInstance) {
    menu.querySelectorAll("select[data-action=setFontSize]").forEach((select) => {
      select.addEventListener("change", (event) =>
        this.onFontSizeChange(menuInstance, event)
      );
    });
  }

  onFontSizeChange(menuInstance, event) {
    event.preventDefault();

    const selectedSize = event.currentTarget.value;
    if (!selectedSize) return;

    const { state, dispatch } = menuInstance.view;
    const { selection } = state;
    const markType = menuInstance.schema.marks.span;
  ...
  getPreserve(state, markType, selection) {
    ...
    state.doc.nodesBetween(selection.from, selection.to, (node) => {
      const mark = node.marks.find(
        (m) => m.type.name === markType.name && m.attrs?._preserve
      );
      if (mark) {
